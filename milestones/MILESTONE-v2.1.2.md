...
#### ⚡ 性能优化：减少 Redis 查询
优化 `determineFlowType` 调用，避免重复的 Redis 查询：

- **问题**: 回复消息时重新调用 `determineFlowType(target, source)`，参数是 forward 的对调，导致重复查询
- **解决**: 添加 `reverseFlowType()` 函数，直接计算对调的 flowType
  - H2A → A2H
  - A2H → H2A
  - A2A → A2A
  - H2H → H2H
- **效果**: 减少 2 次 Redis 查询 per 消息

### 技术细节

**修改文件:**
- `src/core/sessions-send.ts`:
  - 添加 `fromType: 'inner'` 到 forwardMsg 和 replyMessage
  - 添加 `reverseFlowType()` 辅助函数
  - 提升 `flowType` 到更高作用域供回复复用
  - 使用 `reverseFlowType()` 替代重复查询
