                ...
                execute: async (_toolCallId, params) => {
                    ...
                    try {
                        ...
                        if (data.code !== 200 || !data.data?.results?.length) {
                            return {
                                content: [{ type: "text", text: '未找到相关知识库内容。' }],
                        ...
                        return {
                            content: [{ type: "text", text: `知识检索失败: ${err.message}` }],
            ...
            context.registerTool({
                ...
                execute: async (_toolCallId, params) => {
                    ...
                    try {
                        ...
                        return {
                            content: [{ type: "text", text: `技能同步失败: ${err.message}` }],
                            details: { success: false, error: err.message }
                        };
                    }
                }
...
async function getLocalAgents(logger) {
    try {
        ...
        const localAgents = wegirlBindings.map((b) => ({
            agentId: b.agentId,
...
function formatResultForReply(action, result) {
    switch (action) {
        case 'list_staffs': {
            ...
            const npcList = agents.filter((s) => s.type === 'npc');
            const lines = ['📋 团队花名册', ''];
            // 🤖 Robots (Agents)
            if (agentList.length > 0) {
                ...
                Object.keys(agentGroups).sort().forEach(instanceId => {
                    ...
                    agentGroups[instanceId].forEach((a) => {
            ...
            if (humanList.length > 0) {
                ...
                Object.keys(humanGroups).sort().forEach(instanceId => {
                    lines.push(`${instanceId} 实例：`);
                    humanGroups[instanceId].forEach((h) => {
            ...
            if (npcList.length > 0) {
                ...
                npcList.forEach((n) => {
                    ...
                    if (!npcGroups[instanceId])
                        npcGroups[instanceId] = [];
                    npcGroups[instanceId].push(n);
                });
                Object.keys(npcGroups).sort().forEach(instanceId => {
                    lines.push(`${instanceId} 实例：`);
                    npcGroups[instanceId].forEach((n) => {
