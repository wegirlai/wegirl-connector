...
import { validateOptions, createSessionContext, createSessionKey, isNoReply, generateId } from './utils.js';
 ...
 */
function getRedisUrlFromConfig() {
 ...
 */
function getRedisPasswordFromConfig() {
    const cfg = getWeGirlConfig();
 ...
 * 更新步骤进度
 */
async function updateStepProgress(redis, ctx, agentId, status) {
    ...
    // 统计已完成数
    const completedCount = Object.entries(allStatus).filter(([k, v]) => k.startsWith('agent:') && v === 'completed').length;
 ...
 * 投递消息给本地 Agent
 */
async function deliverToLocalAgent(sessionKey, ctx, message, logger) {
    ...
    const { dispatcher, replyOptions: baseReplyOptions, markDispatchIdle } = runtime.channel.reply.createReplyDispatcherWithTyping({
        deliver: async (payload, info) => {
            ...
            try {
                ...
                if (ctx.taskId)
                    replyMessage.taskId = ctx.taskId;
                if (ctx.stepId)
                    replyMessage.stepId = ctx.stepId;
