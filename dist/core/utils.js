 ...
 * 获取默认 replyTo
 */
export function getDefaultReplyTo(flowType, chatType, source, target) {
    // 群聊：默认回复到群里（target）
    // 单聊：默认回复给 source
 ...
 * 解析 replyTo
 */
export function resolveReplyTo(replyTo, flowType, chatType, source, target) {
    // 如果未指定，使用默认值
 ...
 * 创建 Session 上下文
 */
export function createSessionContext(options, routingId) {
    const chatType = options.chatType || 'direct';
    const replyTo = resolveReplyTo(options.replyTo, options.flowType, chatType, options.source, options.target);
    return {
 ...
 * 创建 Session Key
 */
export function createSessionKey(target, chatType, groupId) {
 ...
 */
export function inferEntityType(flowType, role) {
    switch (flowType) {
        case 'H2A':
