...
import { hasAccount } from './index.js';
...
export class WeGirlTools {
    ...
    async send(params) {
        ...
        if (!hasAccount(targetStaffId)) {
            ...
            if (!freshData || Object.keys(freshData).length === 0) {
                return {
                    content: [{ type: "text", text: `目标 "${target}" 不存在。请确认 staffId 是否正确，或使用 wegirl_query 查询可用列表。` }],
                    isError: true,
                    details: {
                        status: 'error',
        ...
        try {
            ...
            if (isSyncMode) {
                ...
                if (result.status === 'timeout') {
                    return {
                        content: [{ type: "text", text: `请求超时：等待 ${effectiveTimeout} 秒后未收到响应` }],
                        isError: true,
                        details: {
                            status: 'timeout',
                            routingId,
            ...
            return {
                content: [{ type: "text", text: `消息已发送给 ${target}` }],
                details: {
                    status: 'accepted',
                    routingId,
                    duration,
            ...
            return {
                content: [{ type: "text", text: `发送异常: ${error.message}` }],
                isError: true,
                details: {
                    status: 'error',
                    error: error.message,
    ...
    // 发布转交过程事件到 Redis
    async publishRoutingEvent(routingId, stage, data) {
        const event = {
            type: 'routing_event',
            routingId,
            stage,
    ...
    async deliverToAgent(agentId, envelope, params, routingId) {
        ...
        const messageId = await this.redis.xadd(streamKey, 'MAXLEN', '~', 5000, '*', 'data', JSON.stringify(deliveryParams));
    ...
    async deliverToHuman(userId, envelope, params, routingId) {
    ...
    async broadcast(envelope, params, routingId) {
