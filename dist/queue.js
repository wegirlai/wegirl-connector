...
export class PendingQueue {
    ...
    // ========== 核心任务操作 ==========
    async createTask(userId, type, title, description, metadata = {}, priority = 'normal') {
    ...
    async updateTaskStatus(taskId, newStatus, extra) {
        ...
        if (extra?.decision)
            updates.decision = extra.decision;
        if (extra?.decisionBy)
            updates.decisionBy = extra.decisionBy;
        if (extra?.decisionMessage)
            updates.decisionMessage = extra.decisionMessage;
    ...
    async processDecision(taskId, decision, decisionBy, message) {
        return this.updateTaskStatus(taskId, decision === 'approve' ? 'approved' : 'rejected', { decision, decisionBy, decisionMessage: message });
    }
    ...
    async getUserTasks(userId, options = {}) {
        ...
        // 获取所有条目
        let entries = await this.redis.zrange(key, 0, -1);
        // 状态筛选
        if (status) {
    ...
    // ========== 旧接口兼容 ==========
    async enqueue(userId, envelope) {
        await this.createTask(userId, 'notification', envelope.payload?.content?.substring(0, 50) || 'New message', envelope.payload?.content || '', { legacy: true, envelope }, 'normal');
    }
    async dequeue(userId, limit = 10) {
    ...
    calculateScore(status, priority, timestamp) {
