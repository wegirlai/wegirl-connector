...
export class WeGirlTools {
  ...
  async send(params: WeGirlSendParams): Promise<any> {
    ...
    try {
      ...
      if (hasReplyTo) {
        ...
        if (!result.success) {
          return {
            content: [{ type: "text" as const, text: `转发失败: ${result.error || '未知错误'}` }],
            isError: true,
            details: {
              status: 'failed',
              error: result.error,
        ...
        // 转发成功（或正在进行中）
        return {
          content: [{ type: "text" as const, text: `已通知 ${target} 执行，结果将转发给 ${params.replyTo}` }],
          details: {
   ...
   */
  async queryStaff(by: 'id' | 'name' | 'capability', query: string): Promise<any[]> {
    const results: any[] = [];
