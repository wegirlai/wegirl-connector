import { existsSync, mkdirSync, readdirSync, statSync, writeFileSync, rmSync, readFileSync } from 'fs';
 ...
 * 获取本地 skills 目录路径
 */
function getLocalSkillsDir(agentId: string): string {
 ...
 * 调用接口获取远程技能列表
 */
async function fetchRemoteSkills(agentId: string): Promise<SkillInfo[]> {
  ...
  return data.data.skills as SkillInfo[];
}

/**
 * 下载单个技能文件
 */
async function downloadSkillFile(skillId: string, filePath: string): Promise<SkillFile> {
  const baseUrl = getDashboardBaseUrl();
 ...
 * 获取本地 skill 的文件列表
 */
function getLocalSkillFiles(skillDir: string): string[] {
 ...
 * 删除本地技能目录
 */
function removeLocalSkill(skillDir: string, skillId: string, logger?: any) {
...
export async function installSkills(
  agentId: string,
  options?: { forceUpdate?: boolean; logger?: any }
  ...
  try {
    ...
    // 4. 处理本地有但远程没有的技能 → 删除
    const removedResults: InstallResult[] = [];
    ...
    // 5. 逐个同步远程技能
    const syncResults: InstallResult[] = [];
 ...
 * 格式化同步结果为易读的文本
 */
export function formatSyncResult(result: SkillSyncResult): string {
