...
const activeCleanupWorkers = new Set<string>();
...
interface ConsumerParams extends MonitorParams {
  consumerName: string;
  streamKey: string;
 ...
 */
async function startCleanupWorker(params: { instanceId: string; cfg: any; abortSignal?: AbortSignal; log?: any }): Promise<void> {
 ...
 */
async function startSingleConsumer(params: ConsumerParams): Promise<void> {
...
export async function monitorWeGirlProvider(params: MonitorParams): Promise<void> {
  ...
  if (!activeCleanupWorkers.has(instanceId)) {
    ...
    startCleanupWorker({ instanceId, cfg, abortSignal, log }).catch((err: any) => {
