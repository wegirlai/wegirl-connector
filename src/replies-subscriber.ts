...
export class RepliesSubscriber {
  ...
  private logger: any;
  private messageHandler: (msg: ReplyMessage) => Promise<void>;

  constructor(
    redisUrl: string,
    redisPassword: string | undefined,
    logger: any,
    messageHandler: (msg: ReplyMessage) => Promise<void>
  ) {
  ...
  async start(): Promise<void> {
    ...
    this.subscriber.on('message', async (channel, message) => {
      try {
        const data: ReplyMessage = JSON.parse(message);
  ...
  private async handleOnboardHuman(data: ReplyMessage): Promise<void> {
