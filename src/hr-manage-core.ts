...
function generateAgentSoulMd(agentName: string, role: string): string {
...
// 生成 BOOTSTRAP.md 模板
function generateBootstrapMd(agentName: string): string {
  const displayName = agentName.charAt(0).toUpperCase() + agentName.slice(1);
...
// 生成 IDENTITY.md 模板
function generateIdentityMd(agentName: string): string {
  const displayName = agentName.charAt(0).toUpperCase() + agentName.slice(1);
...
export async function executeCreateAgent(
  ...
  try {
    ...
    const agentExists = config.agents.list.some(
      (a: any) => a.id === agentName
    );
    if (!agentExists) {
